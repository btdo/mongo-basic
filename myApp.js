...
var findOneByFood = function(food, done) {
  Person.findOne({'favoriteFoods': food}).exec(done);  
};
...
var findPersonById = function(personId, done) {
  ...
  Person.findById(personId).exec(done);  
...
var queryChain = function(done) {
  ...
  Person.find({'favoriteFoods': foodToSearch}).sort({name : 'ascending'}).limit(2).select('name favoriteFoods').exec(done);
